...
function skillAdded() {
  $("#success-skill").show().delay(1000).fadeOut();
}

function skillExist() {
  $("#error-skill").show().delay(1000).fadeOut();
}
...
function addNewSkill(name) {
  $('#badge-list-2').append('<div class="badge badge-pill badge-' + randomColor() + ' draggable">' + name + '</div>');   
}
...
$('#skills-submit').on('click', function () {
  var skills = [];
  //get length of the parent element
  var length = $('#badge-list-1').children('.badge-pill').length;

  for (i = 0; i < length; i++) {
    //get element innertext
    var skill = $('#badge-list-1').children('.badge-pill')[i].innerText;
    //push it into array
    skills.push(skill);
  }

  //convert array to json
  var myJsonString = JSON.stringify(skills);

  $('#skill-json').val(myJsonString);
